        ...
        describe StockLocationsDuplicator do
          describe '#handle_clone_stock_locations' do
            it 'duplicates vendor stock locations from the source vendor' do
              ...
              stock_locations_scope = instance_double('ActiveRecord::Relation')
              ...
              allow(Spree::StockLocation).to receive(:where).and_return(stock_locations_scope)
              allow(stock_locations_scope).to receive(:not).and_return(stock_locations_scope)
              allow(stock_locations_scope).to receive(:where).with(name: 'Main Warehouse').and_return(stock_locations_named_scope)
            ...
            it 'creates a fallback default stock location when the source store has none' do
              ...
              allow(Spree::StockLocation).to receive(:where).and_return(stock_locations_scope)
              allow(stock_locations_scope).to receive(:not).and_return(stock_locations_scope)
              allow(stock_locations_scope).to receive(:where).with(name: 'US location').and_return(stock_locations_named_scope)
              allow(stock_locations_named_scope).to receive(:exists?).and_return(false)
