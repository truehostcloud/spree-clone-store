      ...
      module Duplicators
        class ProductsDuplicator
          ...
          def initialize(old_store:, new_store:)
            @old_store = old_store
            @new_store = Spree::Store.includes(:taxonomies).find_by(strore_id: new_store.id)
          end

          def handle_clone_products
            @new_store = Spree::Store.includes(:taxons).find_by(taxonomy_id: @new_store.taxonomies.all[0].id)
            @old_store.products.map { |product| clone_product(old_product: product) }
